// Dashboard.js (connected to real backend data)
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Fetch transactions for charts and recent view
    fetch(`http://localhost:5000/api/transactions`, { headers })
      .then(res => res.json())
      .then(data => {
        setTransactions(data);

        const incomeGroups = data.filter(tx => tx.type === 'income')
          .reduce((acc, tx) => {
        ...
        const expenseGroups = data.filter(tx => tx.type === 'expense')
          .reduce((acc, tx) => {
            acc[tx.category] = (acc[tx.category] || 0) + parseFloat(tx.amount);
            return acc;
          }, {});
